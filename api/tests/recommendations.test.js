...
module.exports = [
  {
    name: 'pair compatibility rewards semantically aligned tailoring over mismatched casual pieces',
    run() {
      const silkShirt = {
        _id: 'silk-shirt',
      ...
      assert.ok(aligned > mismatched + 0.12);
    },
  },
  {
    name: 'makeCandidate attaches semantic cohesion and embedding data',
    run() {
      const outfitItems = [
        {
          _id: 'blazer',
      ...
      assert.equal(candidate.semanticEmbedding.length, 10);
      assert.ok(candidate.semanticCohesion >= 0 && candidate.semanticCohesion <= 1);
      assert.ok(candidate.score >= 0.4 && candidate.score <= 0.96);
      assert.ok(Array.isArray(candidate.reasons));
    },
  },
  {
    name: 'formatResults rejects semantically duplicated outfits even when core items are unique',
    run() {
      const outfitAItems = [
        {
          _id: 'a-top',
      ...
      const outfitBItems = [
        ...
        },
        {
          ...
          pattern: 'solid',
          fit: 'slim',
          tags: ['tailored', 'polished', 'classic'],
        },
        {
          _id: 'b-shoe',
          name: 'Cream Pump',
      ...
      const outfitCItems = [
        ...
        },
        {
          ...
          category: 'bottom',
          subcategory: 'wide trouser',
          color: 'charcoal',
          style: 'modern',
          pattern: 'solid',
          fit: 'relaxed',
          tags: ['textured', 'modern', 'relaxed'],
        },
        {
          _id: 'c-shoe',
          name: 'White Sneaker',
      ...
      assert.deepEqual(topIds, ['a-top', 'c-top']);
    },
  },
  {
    name: 'appendFallbackResponses fills missing AI results up to the requested limit',
    run() {
      const aiPicked = [
        {
          id: 'hybrid-1',
      ...
      assert.deepEqual(
        ...
        ['hybrid-1', 'fallback-2', 'fallback-3'],
      );
    },
  },
];
