...
function makeRecommendationId(prefix, seed, slot) {
  // A quick local hasher just for the ID generation since it's decoupled
  let hash = 2166136261;
  const str = String(seed + ':' + slot);
...
function normalizeRecommendationScore(value) {
  ...
  if (numeric <= 1.01) return Math.round(Math.max(0, Math.min(1, numeric)) * 100);
  return Math.round(Math.max(0, Math.min(100, numeric)));
}

// Ensure the helper functions from engine are available if needed internally inside AI
// ═══════════════════════════════════════════════════════════════════════════
